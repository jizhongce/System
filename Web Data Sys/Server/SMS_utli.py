...
parameterStr.replace("+", "%20")

#Then replace "+" with "%20"
parameterStr.replace("*", "%2A")

#Then replace "+" with "%20"
parameterStr.replace("%7E", "~")

#Then add POP with HTTPMethod + “&” + specialUrlEncode(“/”) + ”&” + specialUrlEncode(sortedQueryString)
stringToSign = urllib.parse.quote_plus(parameterStr).replace("+", "%20").replace("*", "%2A").replace("%7E", "~")
...
key = bytes(accessSecret + "&", "utf-8")

message = bytes(stringToSign, "utf-8")

digester = hmac.new(key, message, hashlib.sha1)

signature = str(base64.urlsafe_b64encode(digester.digest()), 'UTF-8')

ulrsignature = urllib.parse.quote_plus(signature).replace("+", "%20").replace("*", "%2A").replace("%7E", "~")
